...
import options
import proposal_methods_eval as PM
import pandas as pd
...
def test(itr, dataset, args, model,  device,pool):
    ...
    while not done:
        ...
        with torch.no_grad():
            ...
            results[vn] = {'cas':outputs['cas'],'atn':outputs['atn']}
...
if __name__ == '__main__':
    ...
    model = getattr(model_eval,args.use_model1)(dataset.feature_size, dataset.num_class,opt=args).to(device)
    model.load_state_dict(torch.load('/media/lgz/Scipio/lgz/Bi-SCC/MM2021-CO2-Net5_split2/ckpt/best_atnitr30.75_IAS123_CO2.pkl'))
    pool = mp.Pool(5)

    iou,dmap = test(-1, dataset, args, model,model, device,pool)
